...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 py-6 space-y-5">
          ...
          <Card key={card.label}>
            <CardBody className="flex items-center gap-3 py-4">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide leading-none">
      ...
      <div>
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Impact Streams</h2>
          ...
          <Link to="/audits" className="group">
            <Card className="hover:shadow-md transition-shadow">
              <CardBody className="py-4">
                ...
                <p className="text-xl font-bold text-gray-900">{buildings.filter(b => b.recyclingStatus && b.recyclingStatus !== 'None').length}</p>
                <p className="text-xs text-gray-500 mt-0.5">systems tracked</p>
                ...
                <p className="text-[10px] text-gray-400 mt-2 group-hover:text-teal-600 transition-colors">Go to Audits →</p>
              </CardBody>
            </Card>
          </Link>
          ...
          <Link to="/rwh" className="group">
            <Card className="hover:shadow-md transition-shadow">
              <CardBody className="py-4">
                ...
                <p className="text-xl font-bold text-gray-900">{rwhAssessments.length}</p>
                ...
                <p className="text-[10px] text-gray-400 mt-2 group-hover:text-blue-600 transition-colors">Go to RWH →</p>
              </CardBody>
            </Card>
          </Link>
          ...
          <Link to="/trees" className="group">
            <Card className="hover:shadow-md transition-shadow">
              <CardBody className="py-4">
                ...
                <p className="text-xl font-bold text-gray-900">{treeProjects.reduce((s, p) => s + p.treesPlanted, 0)}</p>
                <p className="text-xs text-gray-500 mt-0.5">trees planted</p>
                <p className="text-xs text-green-600 mt-1">
                  {Math.round(treeProjects.reduce((s, p) => s + p.estimatedCo2PerYearKg, 0) / 1000 * 10) / 10} t CO₂/yr
                </p>
                <p className="text-[10px] text-gray-400 mt-2 group-hover:text-green-600 transition-colors">Go to Trees →</p>
              </CardBody>
            </Card>
          </Link>
          ...
          <Link to="/lakes" className="group">
            <Card className="hover:shadow-md transition-shadow">
              <CardBody className="py-4">
                ...
                <p className="text-xl font-bold text-gray-900">{waterBodies.length}</p>
                <p className="text-xs text-gray-500 mt-0.5">water bodies</p>
                ...
                <p className="text-[10px] text-gray-400 mt-2 group-hover:text-cyan-600 transition-colors">Go to Lakes →</p>
              </CardBody>
            </Card>
          </Link>
        </div>
        ...
        <CardHeader>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-gray-900">Business Phases — How This Works</h2>
            </div>
            <ChevronDown size={16} className={`text-gray-400 transition-transform ${phasesOpen ? 'rotate-180' : ''}`} />
          </button>
        </CardHeader>
        {phasesOpen && (
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl p-4 border-2 ${
                      isActive ? 'border-[#0F6E56] bg-[#0F6E56]/5' :
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className={`text-xs font-bold uppercase tracking-wide mb-0.5 ${
                          isActive ? 'text-[#0F6E56]' : isLocked ? 'text-gray-400' : 'text-green-700'
                        }`}>
                          Phase {p.num} · {p.timeline}
                        </div>
                        <h3 className="font-bold text-gray-900">{p.title}</h3>
                      </div>
                      {isLocked && <span className="text-[10px] bg-gray-200 text-gray-500 px-2 py-0.5 rounded-full font-semibold">Locked until {p.unlockAt}</span>}
                      {isActive && <span className="text-[10px] bg-[#0F6E56] text-white px-2 py-0.5 rounded-full font-semibold">You are here</span>}
                    ...
                    <p className="text-xs text-gray-600 leading-relaxed mb-3">{p.what}</p>
                    <div className="space-y-2 text-xs">
                      <div className="flex gap-2"><span className="text-gray-400 font-semibold w-16 flex-shrink-0">Capital:</span><span className="text-gray-700">{p.capital}</span></div>
                      <div className="flex gap-2"><span className="text-gray-400 font-semibold w-16 flex-shrink-0">You earn:</span><span className="text-gray-700">{p.earn}</span></div>
                      <div className="flex gap-2"><span className="text-gray-400 font-semibold w-16 flex-shrink-0">Risk:</span><span className="text-gray-700">{p.risk}</span></div>
                    </div>
                    {isActive && (
                      <div className="mt-3 border-t border-[#0F6E56]/20 pt-3">
                        <p className="text-[10px] font-bold text-[#0F6E56] uppercase tracking-wide mb-1.5">Your next steps</p>
                        <ul className="space-y-1">
                          {p.nextSteps.map((s, i) => (
                            <li key={i} className="text-xs text-gray-700 flex gap-1.5 items-start">
                              <span className="text-[#0F6E56] font-bold flex-shrink-0">→</span>{s}
                            </li>
                          ))}
                        </ul>
                      </div>
        ...
        <CardHeader>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-gray-900">How a Building Becomes Revenue — 7 Stages</h2>
            </div>
            <ChevronDown size={16} className={`text-gray-400 transition-transform ${journeyOpen ? 'rotate-180' : ''}`} />
          </button>
        </CardHeader>
        {journeyOpen && (
              ...
              {JOURNEY_STAGES.map((s, i) => (
                <div key={s.num} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    {i < JOURNEY_STAGES.length - 1 && <div className="w-px bg-gray-200 flex-1 mt-1" />}
                  </div>
                  <div className="pb-3 min-w-0">
                    <div className="font-semibold text-sm text-gray-900">{s.title} <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${s.color}`}>{s.sub}</span></div>
                    <p className="text-xs text-gray-600 mt-1 leading-relaxed">{s.what}</p>
                    <p className="text-xs text-[#0F6E56] mt-1 font-medium">In JalDhara: {s.jaldhara}</p>
                ...
                <React.Fragment key={s.num}>
                  <div className="flex-shrink-0 w-44 xl:w-48">
                    ...
                    <div className="px-1 mt-2 space-y-1.5">
                      <p className="text-xs text-gray-600 leading-relaxed">{s.what}</p>
                      <p className="text-xs text-[#0F6E56] font-medium leading-snug">{s.jaldhara}</p>
