...
const PHASE_CARDS = [
  {
    ...
    risk: 'Very low',
    color: 'border-[#0F6E56] bg-[#0F6E56]/5',
...
export default function Onboarding() {
  ...
  const steps = [
    ...
    <div key="welcome" className="flex flex-col items-center text-center max-w-lg mx-auto py-8 px-4">
      <div className="w-20 h-20 bg-[#0F6E56] rounded-3xl flex items-center justify-center mb-6 shadow-lg">
        <Droplets size={40} className="text-white" />
      </div>
      <h1 className="text-3xl font-bold text-gray-900 mb-3">Welcome to JalDhara</h1>
      ...
      <p className="text-gray-500 text-sm mb-8">Let us start by understanding what this app does.</p>
      <button
        onClick={() => setStep(1)}
        className="flex items-center gap-2 bg-[#0F6E56] text-white px-8 py-3.5 rounded-2xl font-semibold text-base hover:bg-[#0c5844] transition-colors shadow-sm"
      >
    ...
    <div key="phases" className="max-w-3xl mx-auto py-8 px-4">
      <div className="text-center mb-8">
        <p className="text-xs font-bold text-[#0F6E56] uppercase tracking-widest mb-2">Step 1 of 3</p>
        <h2 className="text-2xl font-bold text-gray-900">Your business has three phases</h2>
        <p className="text-gray-500 text-sm mt-2">You start Phase 1 today. Phases 2 and 3 unlock as you grow.</p>
      </div>
        ...
        {PHASE_CARDS.map(p => (
          <div key={p.num} className={`rounded-2xl border-2 p-5 ${p.color} ${!p.active ? 'opacity-60' : ''}`}>
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <div className="text-xs text-gray-500 font-semibold mb-0.5">{p.timeline}</div>
                <h3 className="font-bold text-gray-900 text-lg">Phase {p.num} — {p.title}</h3>
              </div>
              <div className="flex flex-col items-end gap-1">
                {p.active
                  ? <span className="text-xs bg-[#0F6E56] text-white px-3 py-1 rounded-full font-semibold">You are here</span>
                  : <span className="text-xs bg-gray-200 text-gray-500 px-3 py-1 rounded-full font-semibold flex items-center gap-1"><Lock size={10} /> Locked</span>
                }
              </div>
            </div>
      ...
      <div className="flex justify-center">
        <button
          onClick={() => setStep(2)}
          className="flex items-center gap-2 bg-[#0F6E56] text-white px-8 py-3.5 rounded-2xl font-semibold text-base hover:bg-[#0c5844] transition-colors"
        >
    ...
    <div key="tasks" className="max-w-lg mx-auto py-8 px-4">
      <div className="text-center mb-8">
        <p className="text-xs font-bold text-[#0F6E56] uppercase tracking-widest mb-2">Step 2 of 3</p>
        <h2 className="text-2xl font-bold text-gray-900">Your first task this week</h2>
      </div>
        ...
        {FIRST_TASKS.map(task => (
          ...
          >
            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
              checked[task.id] ? 'border-[#0F6E56] bg-[#0F6E56]' : 'border-gray-300'
            }`}>
      ...
      <div className="flex flex-col gap-3">
        <button
          ...
          className="flex items-center justify-center gap-2 bg-[#0F6E56] text-white px-8 py-3.5 rounded-2xl font-semibold hover:bg-[#0c5844] transition-colors"
        >
          Take me to Scripts <ArrowRight size={18} />
        </button>
        <button onClick={() => setStep(3)} className="text-sm text-gray-400 hover:text-gray-600 py-2 transition-colors">
          Skip for now →
        </button>
      </div>
    </div>,
    ...
    <div key="profile" className="max-w-lg mx-auto py-8 px-4">
      <div className="text-center mb-8">
        <p className="text-xs font-bold text-[#0F6E56] uppercase tracking-widest mb-2">Step 3 of 3</p>
        <h2 className="text-2xl font-bold text-gray-900">Quick setup</h2>
        <p className="text-gray-500 text-sm mt-2">2 minutes. You can change these anytime in Settings.</p>
      </div>
      <div className="space-y-4 mb-8">
        <div>
          ...
          <input
            type="text"
            className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0F6E56]/20"
            value={profile.name}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Mapbox token <span className="text-gray-400 font-normal">(optional — for map view)</span>
          </label>
          <input
            type="text"
            className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0F6E56]/20 font-mono"
          ...
          <p className="text-xs text-gray-400 mt-1.5">
            Sign up free at{' '}
            <a href="https://account.mapbox.com/access-tokens/" target="_blank" rel="noopener noreferrer" className="text-[#0F6E56] underline">
              mapbox.com
            </a>
      ...
      <button
        onClick={complete}
        className="w-full flex items-center justify-center gap-2 bg-[#0F6E56] text-white px-8 py-4 rounded-2xl font-semibold text-base hover:bg-[#0c5844] transition-colors shadow-sm"
      >
      ...
      {step > 0 && (
        <div className="flex justify-center gap-2 pt-6">
          {[0, 1, 2, 3].filter(s => s > 0).map(s => (
            <div
              key={s}
              className={`w-2 h-2 rounded-full transition-all ${s === step ? 'bg-[#0F6E56] w-4' : s < step ? 'bg-[#0F6E56]/40' : 'bg-gray-200'}`}
            />
          ))}
        </div>
      )}
